<x-layouts.main>
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 max-w-2xl mx-auto">
        <div class="text-center mb-6">
            ...
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white mb-2">Замовлення успішно оформлено!</h1>
            <p class="text-gray-600 dark:text-gray-400">Дякуємо за ваше замовлення. Ми зв'яжемося з вами найближчим часом.</p>
        </div>

        @if(isset($order))
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Дані замовлення:</h2>
                <div class="space-y-3">
                    @if(!empty($order['name']))
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Ім'я:</span>
                            <span class="text-gray-900 dark:text-white font-medium">{{ $order['name'] }}</span>
                        </div>
                    @endif
                    @if(!empty($order['messenger']))
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Месенджер:</span>
                    ...
                    @if(!empty($order['contact']))
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Контакт:</span>
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="text-gray-900 dark:text-white font-medium">{{ $order['product_name'] ?? 'Друкована книга' }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600 dark:text-gray-400">Сума:</span>
                        <span class="text-gray-900 dark:text-white font-medium">
                    ...
                    @if(!empty($order['invoice_id']))
                        <div class="flex justify-between">
                            <span class="text-gray-600 dark:text-gray-400">Номер замовлення:</span>
